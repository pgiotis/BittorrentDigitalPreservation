    ...
    if (isset($_FILES["torrentfile"]) && $processuploadedtorrent) {
        ...
        switch ($_FILES["torrentfile"]["error"]) {
            case UPLOAD_ERR_OK: //file uploaded ok
                 ...
                 */
                if (!$copiedTorrent)
                    unlink($_FILES["torrentfile"]["tmp_name"]);
                break;
    ...
    if (!$addError) {
        ...
        if (!isDate($removeurldate, true) || !isDate($hidetorrentdate, true)) {
            ...
            if (!get_magic_quotes_gpc()) {
                ...
                $threshold = addslashes(htmlentities(stripslashes($threshold)));
                $strategy = addslashes(htmlentities(stripslashes($strategy)));
...
?>
                                        ...
                                        <TR>
                                            <TD>Seeds threshold:</TD>
                                            <TD><INPUT TYPE=text NAME="threshold" SIZE=40 MAXLENGTH=200 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["threshold"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT style="font-family:arial;color:red;"> <b>Add a seeders threshold that you want to act Digital Preservation.</b></FONT></TD>
                                        </TR>
                                        <TR>
                                        ...
                                        <TR>
                                            <TD>File name:</TD>
                                            <TD><INPUT TYPE=text NAME="filename" SIZE=40 MAXLENGTH=200 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["filename"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT SIZE=-1><B>(Optional)</B></FONT> If you want to use a different name, enter it here.</TD>
                                        </TR>
                                        <TR>
                                            <TD>URL to torrent:</TD>
                                            <TD><INPUT TYPE=text NAME="url" SIZE=40 MAXLENGTH=200 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["url"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT SIZE=-1><B>(Optional)</B></FONT> Enter the URL to the torrent.</TD>
                                        </TR>
                                        <TR>
                                            <TD>Mirror for torrent:</TD>
                                            <TD><INPUT TYPE=text NAME="urlmirror" SIZE=40 MAXLENGTH=200 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["urlmirror"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT SIZE=-1><B>(Optional)</B></FONT> Enter the URL to the mirror site.</TD>
                                        </TR>
                                        <TR>
                                            <TD>URL to SFV file:</TD>
                                            <TD><INPUT TYPE=text NAME="sfv" SIZE=40 MAXLENGTH=200 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["sfv"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT SIZE=-1><B>(Optional)</B></FONT> Enter the URL to the SFV file.</TD>
                                        </TR>
                                        <TR>
                                        ...
                                        <TR>
                                            <TD>Description:</TD>
                                            <TD><INPUT TYPE=text NAME="shortdesc" SIZE=40 MAXLENGTH=200 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["shortdesc"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT SIZE=-1><B>(Optional)</B></FONT> Enter a short description.</TD>
                                        </TR>
<?php
if ($_SESSION["admin_perms"]["root"])
    if ($addError) {
...
?>
                                        <TR>
                                            ...
                                            <TD><INPUT TYPE=text NAME="crcinfo" SIZE=40 MAXLENGTH=254 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["crcinfo"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT SIZE=-1><B>(Optional)</B></FONT> Enter CRC32 checksum information.</TD>
                                        </TR>
                                        <TR>
                                            <TD>Remove URL:</TD>
                                            <TD><INPUT TYPE=text NAME="removeurl" SIZE=15 MAXLENGTH=254 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["removeurl"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT SIZE=-1><B>(Optional)</B></FONT> Enter a date (format: yyyy-mm-dd) to remove the URL from the index page.</TD>
                                        </TR>
                                        <TR>
                                            <TD>Hide from index:</TD>
                                            <TD><INPUT TYPE=text NAME="indexhidetorrent" SIZE=15 MAXLENGTH=254 <?php if ($addError) {
                                            echo "VALUE=\"${_POST["indexhidetorrent"]}\"";
                                        } ?>>&nbsp;&nbsp;<FONT SIZE=-1><B>(Optional)</B></FONT> Enter a date (format: yyyy-mm-dd) to hide the URL from the index page. <B>Note: Torrent stays active.</B></TD>
                                        </TR>
                                        <?php
                                        /*
